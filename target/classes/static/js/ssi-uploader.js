...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $chooseBtn = $('' +
         ...
         '<button class="ssi-button success choose">' + this.language.chooseFiles + '</button>' +
         '</span>').append(element);
        var $uploadBtn = $('<button id="ssi-uploadBtn" class="ssi-button success ssi-hidden allUp" >' +
         '<span class="ssi-btnIn">' + this.language.upload + '&nbsp;</span>' +
         '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
        var $clearBtn = $('<button id="ssi-clearBtn" class="ssi-hidden ssi-button info close" >' + this.language.clear +
         '</button>');
        ...
        this.$element.append($('<div class="ssi-buttonWrapper clear">').append($chooseBtn, $abortBtn, $uploadBtn, $clearBtn));
        var $uploadBox;
        if (!this.options.preview) {
            ...
            $uploadBox = $('<div class="hkdl-shangchuan-box"><div id="ssi-previewBox" class="clear ssi-uploadBox ssi-previewBox ' + (this.options.dropZone ? 'ssi-dropZonePreview ssi-dropZone"><div id="ssi-DropZoneBack">' + this.language.drag + '</div>' : '">') + '</div></div>');
            this.$element.prepend($uploadBox);
        }
        var thisS = this;
...
//----------------------------UPLOADFILES------------------------------------
//      判断个数
            $uploadBtn.click(function () {// uplssoad the files
                //alert(tableListLength)
                if (tableListLength==1) {
                    voucherUrlArr.length=0;
                    thisS.uploadFiles();
                    $('.hkdl-shangchuan .text strong').hide();
                }else{
                    $('.hkdl-shangchuan .text strong').show();
                }
            });
    ...
    Ssi_upload.prototype.toUploadFiles = function (files) {
        ...
        for (var i = 0; i < filesLength; i++) {
            ...
            if ($.inArray(ext, this.options.allowed) === -1) { // if requested file not allowed
                ...
                tableListLength=this.currentListLength
            } else {
                if (length > filesLength) {
                    filesLength++;
                } else {
            ...
            if (thisS.options.preview) {
                var getTemplate = function (content) {
                    return '<table class="ssi-imgToUploadTable ssi-pending">' +
                     ...
                     '<tr><td class="text">' + cutFileName(filename, ext, 15) + '</td></tr>' +
                     '<tr><td style="height:20px;"><div id="ssi-uploadProgress' + index + '" class="ssi-hidden ssi-uploadProgress"></div></td></tr>' +
                     '<tr style="height:20px !important;"><td style="height:20px;"><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn"><span class="trash10 trash"></span></button></td></tr></table>'
                };
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                voucherUrlArr.push(responseData)
    ...
    var getCurrentListLength = function (thisS) { //get the list length
       // console.log(thisS.inProgress + thisS.successfulUpload + thisS.aborted + thisS.abortedWithError + thisS.pendingss)
    ...
    var locale = {
        en: {
            ...
            clear: '',
            drag: '拖拽文件到这里...',
            sizeError: '文件大小不得超过 $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '文件格式为 $1 请确认上传的格式',//$1=file extension ie(exe files are not supported)
